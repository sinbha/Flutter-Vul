...
class QuickAddTeamScreen extends StatelessWidget {

  QuickAddTeamScreen(TeamMode teamMode, int nMax, Auth owner, [dynamic team]){
    _teamMode = teamMode;
    _nMax = nMax;
    _owner = owner;
...
class TeamCardState extends State<TeamCard> {
  ...
  Future<void> _submit() async{
    ...
    try {
      _team.name = _teamData['name']!;
      _team.idTeamLeader = _owner.userId;
      _team.users.add(_owner);
      _team.users.addAll(players);
  ...
  @override
  Widget build(BuildContext context) {
    if(_teamMode == TeamMode.edit && !init){
      _team.name = _teamData['name']!;
      players = [..._team.users];
      if(players.isNotEmpty) players.removeAt(0);
      init = true;
    }
    final deviceSize = MediaQuery.of(context).size;
    return Column(
      ...
      children: <Widget>[
        ...
        Card(
          ...
          elevation: 8.0,
          child: Container(
            height: players.length > 1 ? 370 : 300,
                ...
                child: Column(
                  children: <Widget>[
                    TextFormField(
                      ...
                      initialValue: _teamMode == TeamMode.edit ? _team.name : null,
                    ...
                    Row(
                      ...
                      children: [
                        SizedBox(
                          height: 250 - (130 / (players.isEmpty ? 1 : players.length)),
                        ...
                        if(players.length + 1 < _nMax) IconButton(onPressed: () async {
                          ...
                          List<String> playersInTournament = _tournamentId == '' ? [] : await Provider.of<Team>(context, listen: false).getPlayersInTournament(_tournamentId);
